...
<script>
	...
	export default {
		...
		computed: {
			...
			payDesc () {
				return this.totalPrice === 0 ? `￥${this.minPrice}元起送` : this.totalPrice < this.minPrice ? `还差￥${this.minPrice - this.totalPrice}元起送` : '去结算'
			},
			payClass () {
			...
			beforeDrop (el) {
				...
				while (count--) {
					...
					if (ball.show) {
						...
						el.style.transform = `tanslate3d(0, ${y}px, 0)`
